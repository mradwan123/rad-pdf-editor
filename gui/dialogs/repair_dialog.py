...
class RepairDialog(BaseToolDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(self.tr("Repair"), parent)

        self._source_path: Path | None = None
        self._source_button = QPushButton(self.tr("Choose PDF to Repair..."))
        self._source_button.clicked.connect(self._choose_source)
        self.add_row(self.tr("PDF file"), self._source_button)

    def _choose_source(self) -> None:
        path_str, _selected_filter = QFileDialog.getOpenFileName(
            self,
            self.tr("Choose PDF file"),
            "",
    ...
    def values(self) -> dict[str, Any]:
        if self._source_path is None:
            raise OperationError("No PDF file selected.")
